...
gulp.task('one', async () => {
  ...
  function findCloseTagIndex(nodes, node) {
    ...
    let TagIndex = 0;
    // 找到闭合标签后的位置
    let otherBrotherNodeStartIndex = 0;
    ...
    for (let i = 0, l = selectNodes.length; i < l; i++) {
      ...
      if (i == 0) {
        if (findNodes && findNodes.select) {
          ...
          newFinds = [];
        } else if (selectNodes[i][0] == '#' || selectNodes[i][0] == '.') {
        ...
        adjacentNodes.forEach((node) => {
          ...
          if (otherBrotherNode.length > 0) {
            ...
            if (oldFindsLength != newFinds.length) {
              ...
              const otherBrotherNodeStartIndex = findCloseTagIndex(brothers, brothers[brotherIndex]);
  ...
  // 检查兄弟选择器是否生效
  const checkChildSelectQuery = (classSelects, findNodes = null) => checkSelectQuery(classSelects, findNodes, 'child');
...
const getWxmlTree = (data, isTemplateWxml = false, mianSelectNodes = { __tag__: {} }, templatePath) => {
  ...
  const getTernaryExpressionClass = (dynamicClass) => {
    ...
    let hasExpression = dynamicClass; const
      expressions = [];
...
// 把Wxml字符串转为树结构
const getTemplateWxmlTree = async (temkey, wxmlStr, selectNodes, templatePath) => await getWxmlTree(wxmlStr, true, selectNodes, templatePath);
